...
def get_money(s, t):
    lis = s.strip(" ").replace(",", ".").split(".")
    if t == "Crédit":
        m = 1
    elif t == "Débit":
        m = -1

    if len(lis) == 1 and lis[0].isdigit():
        return m * (100 * int(lis[0]))
    elif len(lis) == 2 and (not lis[0] or lis[0].isdigit()) and lis[1].isdigit():
        return m * (100 * int("0" + lis[0]) + int(lis[1][:2].ljust(2, "0")))
    else:
...
def get_date(s):
    ...
    if (
        len(lis) == 3
        and lis[1] in var["months"]
        and int(lis[0]) in range(1, var["months"][lis[1]] + 1)
    ):
        lis.reverse()
        return "-".join(lis)
    else:
...
def change_payment(db, m_id, m_form, o_date, o_amount):
    ...
    if m_amount is not None and m_date is not None:
        db.execute(
            """UPDATE payments SET date = ?, name = ?, amount = ?
                      WHERE id = ?""",
            (m_date, m_name, m_amount, m_id),
        )
...
def update_day(db, p_date, p_amount):
    ...
    db.execute(
        "UPDATE total SET amount = amount + ? WHERE date >= ?", (p_amount, p_date)
    )
...
def draw_command(command, c_x):
    ...
    stdscr.addstr(my - 2, 1, "(" + var["mode"].upper() + ")", crs.color_pair(5))
    stdscr.addstr(my - 2, 10, command[5 - mx :])
    stdscr.move(my - 2, 10 + c_x)


def draw_window(b=True, i=True, p=True, log=True, c=True, cm="", c_x=0):
    if b:
        draw_background()
    if var["opened_db"] != "":
        if i:
...
def _mod():
    if var["opened_db"] != "" and var["id_list"] != []:
        ...
        m_form.add_text(
            "Montant de l'opération", "{0:9.2f}".format(abs(o_amount / 100)).strip()
        )
...
def get_command(com=""):
    ...
    while stay:
        ...
        if ch == 410:  # Window resized
            ...
            elif type(ch) is str and ch != "" and ch != "\x7f":
